...
class M3Net(MessagePassing):
    ...
    def sampling(self, x, edge_index, edge_type, train=False):
        ...
        # attention_weight.masked_scatter_(mask_intra, self.attention_intra(concatenated_features[mask_intra]).squeeze())
        # attention_weight.masked_scatter_(mask_inter, self.attention_inter(concatenated_features[mask_inter]).squeeze())
        # attention_weight.masked_scatter_(mask_crossmodal, self.attention_crossmodal(concatenated_features[mask_crossmodal]).squeeze())
    ...
    def forward(self, x, edge_index, edge_type, train=False):
        ...
        if self.denoise:
            ...
            # print(" == 0", torch.sum(denosed_weight==0).item())
            # print(" != 0", torch.sum(denosed_weight!=0).item())
        else:
    ...
    def message(self, x_i, x_j, edge_index, size, denoise_weight=None):
        ...
        # use denoise degree
        if self.denoise:
            denoise_edge_index = edge_index[:,denoise_weight != 0]
            # denoise_edge_index = remove_self_loops(denoise_edge_index)[0]
            # denoise_edge_index = add_self_loops(denoise_edge_index, num_nodes=size[0])[0]
            row, col = denoise_edge_index
            deg = degree(row, size[0], dtype=x_j.dtype)
            deg_inv_sqrt = deg.pow(-0.5)
            deg_inv_sqrt = torch.clamp(deg_inv_sqrt, 0.0, 1.5)
        ...
        alpha_g = torch.tanh(self.gate(torch.cat([x_i, x_j], dim=1)))#e.g.[135090, 1]
        if self.denoise:
            return norm.view(-1, 1) * (x_j) * denoise_weight.unsqueeze(1) * alpha_g
        else:

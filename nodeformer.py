...
def create_projection_matrix(m, d, seed=0, scaling=0, struct_mode=False):
    ...
    for _ in range(nb_full_blocks):
        ...
        if struct_mode:
            ...
            unstructured_block = torch.randn((d, d), device='cuda')
    ...
    torch.manual_seed(current_seed)
    if scaling == 0:
        multiplier = torch.norm(torch.randn((m, d), device='cuda'), dim=1)
    elif scaling == 1:
        multiplier = torch.sqrt(torch.tensor(float(d), device='cuda')) * torch.ones(m)
    else:
...
class NodeFormerConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_heads, kernel_transformation=softmax_kernel_transformation, projection_matrix_type='a',
                 nb_random_features=10, use_gumbel=True, nb_gumbel_sample=10, rb_trans='sigmoid', use_edge_loss=True):
    ...
    def forward(self, z, adjs, tau):
        ...
        if self.projection_matrix_type is None:
            ...
            # seed = tensor(2147483647, device='cuda:1', dtype=torch.int32)
            # self.nb_random_features = 30
            projection_matrix = create_projection_matrix(
                self.nb_random_features, dim, seed=seed).to(query.device)
            # projection_matrix.shape = torch.Size([30, 32])
        ...
        # if self.use_gumbel:  # only using Gumbel noise for training
            z_next, weight = kernelized_gumbel_softmax(query,key,value,self.kernel_transformation,projection_matrix,adjs,
                                                  self.nb_gumbel_sample, tau, self.use_edge_loss)
            # z_next.shape = torch.Size([1, 2708, 4, 32])
            # weight.shape = torch.Size([1, 13264, 4])
        else:
            z_next, weight = kernelized_softmax(query, key, value, self.kernel_transformation, projection_matrix, adjs,
                                                tau, self.use_edge_loss)

        # compute update by relational bias of input adjacency, requires O(E)
        # z_next += add_conv_relational_bias(value, adjs, self.b, self.rb_trans)
...
class NodeFormer(nn.Module):
    ...
    '''
    def __init__(self, in_channels, hidden_channels, num_layers=2, num_heads=4, dropout=0.0,
            ...
            self.convs.append(
                NodeFormerConv(hidden_channels, hidden_channels, num_heads=num_heads, kernel_transformation=kernel_transformation,
                              nb_random_features=nb_random_features, use_gumbel=use_gumbel, nb_gumbel_sample=nb_gumbel_sample,
                               rb_trans=rb_trans, use_edge_loss=use_edge_loss))
            self.bns.append(nn.LayerNorm(hidden_channels))
    ...
    def forward(self, x, adjs, tau=1.0):
        # Cora dataset: x.shape = [2708, 1433]
